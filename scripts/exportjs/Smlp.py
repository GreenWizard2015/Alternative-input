"""Export sMLP weights and test data for JavaScript."""
...
def export_smlp(args) -> Dict[str, Any]:
    """Export sMLP weights and test data.
    ...
    layer = sMLP(sizes=[64, 32])

    # Create test inputs to build the layer and create weights
    rng = np.random.RandomState(42)
    test_inputs = {
        "input": stabilize_input(rng.randn(2, 10, 4).astype(np.float32)),
    }
