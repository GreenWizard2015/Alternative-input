"""Export MultiHeadAttention weights and test data for JavaScript."""
...
def export_multiheadattention(args) -> Dict[str, Any]:
    """Export MultiHeadAttention weights and test data.
    ...
    layer = MultiHeadAttention(d_model=64)

    # Create test inputs to build the layer and create weights
    rng = np.random.RandomState(42)
    test_inputs = {
        "query": stabilize_input(rng.randn(2, 10, 64).astype(np.float32)),
        "key": stabilize_input(rng.randn(2, 10, 64).astype(np.float32)),
        "value": stabilize_input(rng.randn(2, 10, 64).astype(np.float32)),
    }

    # Run layer inference first to build the layer and create weights
    layer(
        test_inputs["query"], test_inputs["key"], test_inputs["value"], training=False
    )
    stabilize(layer)
    outputs = layer(
        test_inputs["query"], test_inputs["key"], test_inputs["value"], training=False
